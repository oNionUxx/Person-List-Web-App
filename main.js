...
document.querySelector('body').addEventListener('input', (e) => {
    ...
    e.preventDefault(); let input, current_case;

     // Get form input id name
    if(e.target.parentElement.parentElement.parentElement.parentElement.tagName === `FORM`) {
        input = document.querySelector(`#${e.target.id}`);
        current_case = input.id;
    }
    
    // Get table input class name
    else if(e.target.parentElement.parentElement.parentElement.parentElement.tagName === 'TABLE') {
    ...
    if (input.value != '') {
        ...
        switch(current_case) {
            ...
            case `firstName`: {
                ...
                if(!display_message) message = `Person name must contains at least characters!`;
                break;
            }

            case `lastName`: {
                display_message = Exceptions.isValidName(input.value);
                if(!display_message) message = `Person name contains at least characters!`;
                break;
            }

            case `age`: {
                display_message = Exceptions.isValidAge(input.value);
                if(!display_message) message = `person age must be a valid number and above the age of 18!`;
                break;
            }

            case `ftd`: {
                display_message = Exceptions.isValidFtd(input.value);
                if(!display_message) message = `Person ftd must be a valid number & in the range of (5000$-120000$)`;
                break;
            }
        }
    }
    // Display the user an informative message about his invalid input
    if(!display_message && message != '') UI.showAlert(message, `danger`, `exception`);
    ...
    if(e.target.tagName === 'BUTTON') {
        ...
        for(let i = 0; i < inputs.length; i ++) {
            ...
            // No invalid input was found
                ...
                if(id != '' && firstName != '' && lastName != '' && age != '' && ftd != '') {
                        
                    // Instantiate person
                    const person = new Person(id, first_name, last_name, age, ftd);
                
                    // User action confirmation
                    UI.showAlert(`Are you sure you would like to add ${person.getFullName()}, (id: ${person.id}) ?`,
                                `info`, `add`);
                        ...
                        if(result === 'yes') {
                            ...
                            function checkIfPersonExists(res) {
                                if(res) {
                                    ...
                                    UI.showAlert(`Person id: ${person.id}, ${person.getFullName()} has been successfully added!`,
                                    `success`, `info-message`);
                                }
                                
                                else {
                                    UI.showAlert('Person id is already taken please use another id', `danger`, `info-message`);
                                }
                            }
                    ...
                    UI.showAlert('Form fields cannot remain empty', 'danger', 'info-message');
                } 
            }
        }
    }
...
document.querySelector('#person-list').addEventListener('click' ,(e) => {
    ...
    if(e.target.classList.contains('delete')) {
        ...
        let fullName = rowToBeDeleted.children[1].innerText + ' ' + rowToBeDeleted.children[2].innerText;

         // User action confirmation
         UI.showAlert(`Are you sure you want to delete ${fullName}, (id: ${idValue})`,
         `info`, `delete`);
         ...
         Util.getAnswerState((result) => {
            if(result === `yes`) {
                ...
                UI.showAlert(`Person id: ${idValue} (${fullName}) has been successfully removed`, `success`, `info-message`);
            }
            else if(result === `no`) {
                UI.showAlert(`Person id: ${idValue} (${fullName}) has not been removed`, `info`, `info-message`);
            }
...
document.querySelector('#person-list').addEventListener('click' ,(e) => {
    ...
    let i = 0; let id, firstName, lastName, age, ftd;
    ...
    if(e.target.classList.contains('edit-btn')) {
        ...
        if(row != null) {
            ...
            while(i < row.children.length - 1) {
                ...
                previousCurrentPersonDetails = 
                new Person(id, firstName, lastName, age, ftd);
            }
...
document.querySelector('#person-list').addEventListener('click' ,(e) => {
    ...
    if(e.target.classList.contains('save-btn')) {
        ...
        for(let i = 0; i < list.children.length; i ++) {
            ...
            if(list.children[i].firstElementChild.nextElementSibling.firstElementChild != null) {
                ...
                for(let k = 0; k < personInputs.length; k ++) {
                    ...
                    if(personInputs[k].value === '' || personInputs[k].style.color === `red`) {
                        flag = 0;
                        UI.showAlert(`Input fields are not valid!`, `danger`, `info-message`);
                    ...
                    // End of inputs array - no empty fields were found
                        ...
                        while(j < personInputs.length) {
                            ...
                            if(currentRowId === list.children[i].firstElementChild.innerText) {
                                // Save person to local store
                                Store.updatePerson(new Person(id, first_name, last_name, age, ftd));
...
const show = document.querySelector('.show-selection').addEventListener('change', (e) => {
...
document.querySelector('table thead').addEventListener('click', (e) => {
    ...
    if(e.target.tagName === 'I') {
        ...
        // If the user want to filter in an descending way
            ...
            Util.handleFilter(`.filter-${filter}-down`, `.filter-${filter}-up`, `down`);
        } 
    }
});
...
document.querySelector('#person-list').addEventListener('mouseover', (e) => {
    ...
    if(e.target.parentElement.tagName === 'TR' || e.target.parentElement.tagName === 'A') {
