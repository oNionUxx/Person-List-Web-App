...
class Util {
    ...
    static handleFilter(className_1, className_2, state) {
        ...
        if(document.querySelector(className_2).classList.contains('active-filter')) {
    ...
    static getUpdatedDataList() {
        ...
        if(sorting_state === 'up' || sorting_state === 'down') Sort.mainSort(filter);
    ...
    static getKey(element) {
        let filter_name = element; let str = ''; 
    ...
    static timerFunction = () => {
        ...
        let x = setInterval(function() {
            ...
            if(document.querySelector('#timer') != null) {
                ...
                document.querySelector('#timer').innerHTML = `Your action will be expired in ${seconds}s`;
    ...
    static restoreRow(row) {
        if(row.children[1].firstElementChild != null) {
            ...
            if(id != '' && firstName != '' && lastName != '' && age != '' && ftd != '') {
                ...
                UI.showAlert('Input fields cannot remain empty', 'danger', 'info-message');
                UI.displayPerson();
            }
        }
    }
